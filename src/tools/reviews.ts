...
export function registerReviewTool(host: IcpcHost, ctx: Context): void {
  ctx.tools.register(defineTool({
    ...
    description:
      ...
      'action=remove 移出队列。',
    parameters: {
      action: { type: 'string', required: true, enum: ['list', 'add', 'feedback', 'remove'] },
      ...
      id: { type: 'number', description: '复习条目 ID（feedback/remove）' },
    ...
    async execute(args) {
      ...
      switch (action) {
        case 'list':
          return { ok: true, items: store.listReviews(args.due === true).map((r) => ({
            ...r, intervalDays: intervalDaysForStage(r.stage),
          })) }
        ...
        case 'feedback': {
          ...
          if (!feedback || !['hard', 'ok', 'easy'].includes(feedback))
            return { ok: false, error: "feedback 需为 'hard' | 'ok' | 'easy'" }
          ...
          store.updateReview(id, {
            stage: next.stage, nextDueOn: next.nextDueOn,
